...
const MINOR_THRESHOLD_S = 5 * 60; // 5 minutes
...
export function DomainTable({
  ...
  const totalDay = data.reduce((s, d) => s + d.totalSeconds, 0) || 1;
  const maxSeconds = data[0]?.totalSeconds || 1;

  const major = data.filter((d) => d.totalSeconds >= MINOR_THRESHOLD_S);
  const minor = data.filter((d) => d.totalSeconds < MINOR_THRESHOLD_S);
  const minorSeconds = minor.reduce((s, d) => s + d.totalSeconds, 0);

  const renderRow = (row: DomainRow, i: number, rankIndex: number) => {
    const style = RANK_STYLES[rankIndex] ?? DEFAULT_STYLE;
    const share = (row.totalSeconds / totalDay) * 100;
    const barWidth = (row.totalSeconds / maxSeconds) * 100;
    const siteName = getSiteName(row.domain, row.pageTitle);
    ...
    return (
      <motion.li
        ...
        onClick={() => onDomainClick?.(row.domain, row.totalSeconds)}
      ...
      >
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          <span className="text-sm font-bold tabular-nums text-white">{formatTime(row.totalSeconds)}</span>
      ...
      <div className="flex flex-col gap-1 border-b border-white/[0.07] px-6 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
        <div>
          <div className="flex items-center gap-1.5">
            ...
            <InfoTooltip text="Full ranked list of every domain tracked today. The bar shows each domain's share relative to your top site. Visits counts tab switches to that domain." side="bottom" />
          </div>
        ...
        <div className="flex items-center gap-2 text-[11px] font-medium text-white/30">
          ...
          <span>{data.length} domain{data.length !== 1 ? "s" : ""} tracked</span>
        </div>
      </div>

      {/* Main rows */}
      <ul className="divide-y divide-white/[0.05]">
        {major.map((row, i) => renderRow(row, i, i))}
              ...
              {minorExpanded && (
                ...
                >
                  <ul className="divide-y divide-white/[0.04]">
                    {minor.map((row, i) => renderRow(row, major.length + i, major.length + i))}
                  </ul>
                </motion.div>
              )}
            </AnimatePresence>
