...
export function PastDayView({ state, forDate }: Props) {
  const recurring = rulesForDate(state.recurringRules, forDate);
  ...
  const hasAnything = recurring.length > 0 || adHoc.tasks.length > 0;
    ...
    return (
      <div className="rounded-2xl border border-dashed border-white/10 px-4 py-14 text-center">
        ...
        <p className="mt-1 text-xs text-white/20">
          Ad-hoc tasks and recurring completions would appear here.
        </p>
      </div>
    );
  }
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    ...
                    {r.estimateMinutes != null && (
                      <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] text-white/45 tabular-nums">
                        {r.estimateMinutes}m
                      </span>
                    )}
                  </div>
